...
// private types and functions ------------------------------------------------
  ...
  void printPreOrder(FILE* out, Node R){
    if (R!=NULL){
      fprintf(out, "%d ",R->key);
      printPreOrder(out, R->left);
      printPreOrder(out, R->right);
    }
  }

  void printPostOrder(FILE* out, Node R){
    if(R!=NULL){
      printPostOrder(out,R->left);
      printPostOrder(out,R->right);
      fprintf(out, "%d ",R->key);
    }
  }

  void treeWalk(char* order, Node R){
    if(R == NULL) return;

    if(strcmp(order, "pre")==0){
      printf("%d \n", R->key);
      treeWalk(order, R->left);
      treeWalk(order, R->right);
    }
  ...
  // pre: lookup(D, k)==NULL
  void insert(Dictionary D, int k){
    Node N, A, B;
    if( D==NULL ){
    ...
    if( findKey(D->root, k)!=NULL ){
      fprintf(stderr, "Dictionary Error: cannot insert() duplicate key: \"%d\"\n", k);
      exit(EXIT_FAILURE);
    }
    N = newNode(k);
    B = NULL;
    ...
    while( A!=NULL ){
      B = A;
      if( (k - A->key)<0 ) A = A->left;
      else A = A->right;
    }
    if( B==NULL ) D->root = N;
    else if( (k - B->key)<0 ) B->left = N;
    else B->right = N;
    D->numItems++;
  }
  ...
  // pre: lookup(D, k)!=NULL
  void delete(Dictionary D, int k){
    Node N, P, S;
    if( D==NULL ){
    ...
    N = findKey(D->root, k);
    if( N==NULL ){
      fprintf(stderr, "Dictionary Error: cannot delete() non-existent key: \"%d\"\n", k);
      exit(EXIT_FAILURE);
    }
  ...
  void printDictionary(FILE* out, Dictionary D){
    ...
    printf("print BST in order: ");
    printInOrder(out, D->root);
    printf("\n");
   
    printf("print BST pre order: ");
    printPreOrder(out, D->root);
    printf("\n");

    printf("print BST post order: ");
    printPostOrder(out, D->root);
    printf("\n");
  }
